...
//        setLessonList(getLessonList());
   ...
   /*After Writing Some tests , The system has generated some Topics that need s focussing
   *
   *
   *  show it in 3 recycelr views*/
...
//    we have three models here for 3 recycler views ,
    ...
    public void avgProvider(List<Topic> average, List<Topic> good, List<Topic> bad) {
        ...
        try{
            ...
            //set Adapter
            mGoodlistview.setAdapter(new TopicPerfromingAdapter(getApplicationContext(),good));
            mBadList.setAdapter(new TopicPerfromingAdapter(getApplicationContext(),bad));
            mAvgList.setAdapter(new TopicPerfromingAdapter(getApplicationContext(),average));
